...
class ProofOrI(ProofOverTicks): 
    ...
    def run_example(self):
        ...
        with m.If(Const(1)
            & last.input_ready 
            & last.itype.match(opcode=Opcode.OpImm, funct3=OpImm.OR, rs1=3, rd=2, imm=0b1011) 
            & (last.r[3] == 0b0101)
        ):            
            comb += Assert(now.r[2] == 0b1111)


        with m.If(Const(1)
            & last.input_ready 
            & last.itype.match(opcode=Opcode.OpImm, funct3=OpImm.OR, rd=1, imm=-1)             
        ):            
            comb += Assert(now.r[1] == -1)
    ...
    def run_general(self):        
        ...
        with m.If(last.itype.match(opcode=Opcode.OpImm, funct3=OpImm.OR) & last.input_ready):
