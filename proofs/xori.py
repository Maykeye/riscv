...
class ProofXorI(ProofOverTicks): 
    def __init__(self):
        super().__init__(1)
    ...
    def run_example(self):
        ...
        with m.If(Const(1)
            & last.input_ready 
            & last.itype.match(opcode=Opcode.OpImm, funct3=OpImm.XOR, rs1=3, rd=2, imm=0b1111) 
            & (last.r[3] == 0b0111)
        ):            
            comb += Assert(now.r[2] == 0b1000)


        with m.If(Const(1)
            & last.input_ready 
            & last.itype.match(opcode=Opcode.OpImm, funct3=OpImm.XOR, rs1=1, rd=1, imm=-1)             
        ):            
    ...
    def run_general(self):        
        ...
        with m.If(last.itype.match(opcode=Opcode.OpImm, funct3=OpImm.XOR) & last.input_ready):
